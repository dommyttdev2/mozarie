...
1. 上部の「参照...」を押すと、アプリ内の画像選択画面が開きます。ここでサブフォルダを開き、画像を複数選択できます。何も選ばずに読み込むと、表示中フォルダ配下の PNG / JPEG / WebP を再帰的に一覧へ読み込みます。
2. ギャラリーには複数ファイルをドラッグ＆ドロップできます。ディレクトリをドロップした場合は、その直下にある対応画像だけを追加します。ドラッグした画像は元フォルダへコピーせず、`%TEMP%\LetsCensoring\session-...\imports` の作業用セッションへ原バイト列のまま一時保存します。
...
- **元画像を上書き**: 通常の参照画像へ直接保存します。ドラッグ追加画像を含む場合は選べず、現在の読み込みフォルダへコピー保存します。
...
コピー名が既に存在する場合は、自動で連番を付けます。ドラッグ追加画像の成果物は作業用TEMPではなく、現在の読み込みフォルダへ確定保存するため、セッションの清掃で消えません。保存中は進捗と現在処理中のファイル名を表示し、「一時停止」「再開」「キャンセル」を利用できます。一時停止とキャンセルは、処理中の1枚を安全に終えた後で反映されます。完了した画像は巻き戻しません。
