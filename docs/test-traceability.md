...
| WS-118 | pause/resume/cancelを含む処理状態は操作面と対象数を保って扱う | `tests/test_import_picker_e2e.cjs` | 実モデルの開始・停止、CMDの進捗表示 |

## active

| 手動ID | 実機で確認する理由 |
| --- | --- |
| ED-129 | キャンバスの画像外余白とポインター開始位置は、実ブラウザーのレイアウト、拡大率、比較表示で確認する。 |
| ED-130 | 無名作業の履歴復元中に複数の保存要求を失わせた後の再同期表示は、実ブラウザーと実サーバーの通信順で確認する。 |
