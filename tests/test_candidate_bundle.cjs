...
const context = { state, Map, Set, Math, Promise, AbortController, DOMException, setTimeout, clearTimeout, document: { querySelector() { return null; } }, encodeURIComponent, api: async () => apiResult, isCurrentGeneration: () => true, abortCatalogLoads() { for (const controller of state.catalogLoadControllers) controller.abort(); state.catalogLoadControllers.clear(); state.imageInflight.clear(); state.candidateInflight.clear(); }, catalogRecordMatches: (current, epoch, { version, revision } = {}) => epoch === state.catalogEpoch && current === record && record.assetVersion === version && (revision == null || record.candidateRevision === revision) };
vm.runInNewContext(fs.readFileSync(path.join(__dirname, "..", "static", "js", "resources.js"), "utf8"), context); vm.runInNewContext(`${fs.readFileSync(path.join(__dirname, "..", "static", "js", "editor-canvas.js"), "utf8")}\nglobalThis.loadCandidateBundle = loadCandidateBundle; globalThis.cachedImage = cachedImage;`, context); context.fetchBitmap = (...args) => bitmapLoader(...args);
