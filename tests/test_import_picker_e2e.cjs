...
async function main() {
  ...
  try {
    ...
    for (const [width, language] of [[1024, "ja"], [1920, "en"]]) {
      ...
      assert.equal(editor.targets.count === 2 && editor.targets.native && editor.targets.oneLine && editor.targets.withinPane && editor.targets.compact && editor.targets.selected && editor.targets.tracksAbsent, true, `target chips stay compact and aligned at ${width}/${language}`);
