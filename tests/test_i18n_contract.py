...
class TranslationContractTests(unittest.TestCase):
    ...
    def test_every_emitted_error_code_has_a_user_error_presentation(self) -> None:
        root = Path(__file__).resolve().parents[1]
        source = (root / "static" / "js" / "core.js").read_text(encoding="utf-8")
        aliases = dict(re.findall(r'([a-z_]+):\s*"([a-z_]+)"', source.split("const USER_ERROR_CODES", 1)[1].split("};", 1)[0]))
        emitted: set[str] = set()
        ...
        self.assertEqual(emitted - aliases.keys(), set())
        for language in ("ja", "en"):
            dictionary = json.loads((root / "static" / "i18n" / f"{language}.json").read_text(encoding="utf-8"))
            for emitted_code in emitted:
                presentation_code = aliases[emitted_code]
                for part in ("title", "cause", "action"):
                    self.assertTrue(
                        dictionary[f"errorDialog.{presentation_code}.{part}"],
                        f"{language}: {emitted_code} must reach {presentation_code}.{part}",
                    )
