...
async function fetchBitmap(url, signal) { const response = await fetch(url, { signal, headers: { "X-Mozarie-Token": document.querySelector('meta[name="mozarie-token"]')?.content || "" } }); if (!response.ok) { const payload = await response.json().catch(() => ({})); const localized = payload.error_code ? t(`errorCode.${payload.error_code}`, payload.params || {}) : ""; const error = new Error(localized && localized !== `errorCode.${payload.error_code}` ? localized : t("error.imageLoad")); error.status = response.status; error.code = payload.error_code || ""; throw error; } return createImageBitmap(await response.blob()); }
