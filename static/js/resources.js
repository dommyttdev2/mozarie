...
const isCurrentOrPendingImage = (key) => key.startsWith(`${state.currentId || ""}:`) || key.startsWith(`${state.pendingImageId || ""}:`);
state.imageCache = new WeightedLru(96 * 1024 * 1024, closeBitmap, isCurrentOrPendingImage);
state.candidateBundleCache = new WeightedLru(128 * 1024 * 1024, releaseCandidateBitmapBundle, (key, bundle) => bundle?.candidateImages === state.candidateImages || isCurrentOrPendingImage(key));
...
async function fetchBitmap(url, signal) { const response = await fetch(url, { signal, headers: { "X-Mozarie-Token": document.querySelector('meta[name="mozarie-token"]')?.content || "" } }); if (!response.ok) { const error = new Error(t("error.imageLoad")); error.status = response.status; throw error; } return createImageBitmap(await response.blob()); }
...
function drainPrefetchQueue() { state.prefetchTimer = null; while (state.prefetchActive < 2 && state.prefetchQueue.length) { const { record } = state.prefetchQueue.shift(); state.prefetchActive += 1; cachedImage(record, false).then((image) => { const key = imageCacheKey(record); if (!state.imageCache.has(key) && !isCurrentOrPendingImage(key)) closeBitmap(image); }).catch(() => {}).finally(() => { state.prefetchActive -= 1; drainPrefetchQueue(); }); } }
